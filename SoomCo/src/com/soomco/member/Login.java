...
public class Login extends Page{
	...
	public Login(SoomcoMain MainFrame) {
		super(MainFrame);
		...
		bt_regist.addActionListener((e)->{
			getSoomcoMain().showPage(MainFrame.MEMBER_REGIST);
		});
		
		//로그인 버튼과 리스너연결 
		bt_login.addActionListener((e)->{
			SoomcoMember vo = new SoomcoMember();
	...
	public void validCheck(SoomcoMember shopMember) {
		if(shopMember.getMid().length()<1) {//문자열의 길이가 0이라면..
			...
			if(login(shopMember)==null) {
				...
				//버튼의 라벨을 로그아웃으로 전환
				getSoomcoMain().navi[4].setText("Logout");
				getSoomcoMain().navi[4].setBackground(Color.BLUE);
				getSoomcoMain().navi[4].setForeground(Color.WHITE);
	...
	//null을 반환받아 간다!! 
	public SoomcoMember login(SoomcoMember shopMember) {
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		SoomcoMember vo=null; //로그인 성공 후 회원의 모든 정보를 담게될 VO		
		...
		try {
			...
			//회원정보를  vo에 옮겨담자!!
			if(rs.next()){
				vo = new SoomcoMember(); //회원이 존재할때는 vo 생성
	...
	public void logout() {
		...
		getSoomcoMain().navi[4].setBackground(null);
		getSoomcoMain().navi[4].setForeground(null);
		getSoomcoMain().navi[4].setText("login");
		getSoomcoMain().showPage(SoomcoMain.HOME);
